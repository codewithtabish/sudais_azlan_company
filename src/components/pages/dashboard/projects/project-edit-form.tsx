...
function normalizeContent(value: ProjectContent | null | undefined): ProjectContent {
...
function normalizeTechnologies(values: string[] | null | undefined) {
...
export default function ProjectEditForm({ project }: ProjectEditFormProps) {
  ...
  const [content, setContent] = useState<ProjectContent>(normalizeContent(project.content));
  ...
  const [bannerPreview, setBannerPreview] = useState(project.bannerImage ?? "");

  const [ogImage, setOgImage] = useState(project.ogImageUrl ?? "");

  const [ogPreview, setOgPreview] = useState(project.ogImageUrl ?? "");
  ...
  const [startedAt, setStartedAt] = useState(normalizeDate(project.startedAt));

  const [completedAt, setCompletedAt] = useState(normalizeDate(project.completedAt));
  ...
  useEffect(() => {
    ...
    async function loadCategories() {
      ...
      try {
        ...
        console.error("[ProjectEditForm] Category loading error:", error);
  ...
  const handleBannerUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error("[ProjectEditForm] Banner upload error:", error);
  ...
  const handleOgUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error("[ProjectEditForm] OG upload error:", error);
  ...
  const validateForm = () => {
    ...
    if (shortDescription.trim().length > 500) {
      toast.error("Project short description cannot exceed 500 characters.");
      return false;
    }
  ...
  const handleSubmit = () => {
    ...
    const uniqueTechnologies = Array.from(
      new Set(cleanedTechnologies.map((technology) => technology.toLowerCase())),
    ).map((normalized) => {
      const original = cleanedTechnologies.find(
        (technology) => technology.toLowerCase() === normalized,
      );
    ...
    startTransition(async () => {
      try {
        ...
        console.error("[ProjectEditForm] Update project error:", error);
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">
            Update your project, content, technology, links and publishing settings.
          </p>
        </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Optional. If no OG image is provided, your project banner can be used for social
                  previews.
                </p>
              </CardContent>
            </Card>
